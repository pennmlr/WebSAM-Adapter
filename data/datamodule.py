...
class S3BatchEvalDataset(Dataset): 
    def __init__(self, data_loader, files, batch_size, transform=None): 
        self.data_loader = data_loader 
        self.files = files 
        self.batch_size = batch_size 
    ...
    def __getitem__(self, idx): 
        ...
        for file_key in batch_files:
            ...
            image = image.squeeze(0)
            edges_fine_image.squeeze(0)
            edges_coarse_image.squeeze(0)

            ground_truth_mask_tensor = self.segmentations_to_mask(ground_truth, img_size=(1024, 1024)).squeeze(0)

            dom_nodes_mask_tensor = self.nodes_to_mask(nodes, img_size=(1024, 1024)).squeeze(0)

            data_batch.append((image, ground_truth_mask_tensor, edges_fine_image, edges_coarse_image, dom_nodes_mask_tensor))
